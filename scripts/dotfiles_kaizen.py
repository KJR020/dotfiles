...
class DotfilesKaizen:
    ...
    def read_target_files(
        self, target_patterns: list[str], base_path: str = "."
    ) -> str:
    ...
    def analyze_content(
        ...
        system_prompt = """
You are an expert kaizen advisor specializing in software development best practices.
Your task is to analyze the current configuration files and provide actionable recommendations based on the latest research.

Output your response in the following Markdown format:

## Gap Analysis
(Analyze the gap between current configuration and latest trends)
...
(Step-by-step guide to implement the recommendations)
...
"""
        ...
        user_prompt = f"""
Please analyze the following domain: {domain["name"]}

## Context
- Description: {domain["description"]}
- Current Version/State: {analysis_context.get("current_version", "Unknown")}
- Priority Aspects: {priority_aspects}

## Latest Research Summary (2025)
...
Based on the above, please provide a detailed analysis and recommendations.
"""
...
"""
    ...
    def run_domain_analysis(self, domain_id: str):
        ...
        # ファイルの読み込み
        current_content = self.read_target_files(domain["target_files"])
...
def main():
    ...
    try:
        ...
        kaizen_engine = DotfilesKaizen(config)
